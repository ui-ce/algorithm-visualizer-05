...
const initTsConfigTransformFactory = (defaultTsConfig) => (0, transform_1.transformFromPromise)(graph => {
    // Initialize tsconfig for each entry point
    const entryPoints = graph.filter(nodes_1.isEntryPoint);
    return (0, tsconfig_1.initializeTsConfig)(defaultTsConfig, entryPoints);
});
