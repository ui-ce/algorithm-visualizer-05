import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarTextUnderlineCircleBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarTextUnderlineCircleBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarTextUnderlineCircleBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarTextUnderlineCircleBroken, "svg[solarTextUnderlineCircleBroken]", never, {}, {}, never, never, true, never>;
}
