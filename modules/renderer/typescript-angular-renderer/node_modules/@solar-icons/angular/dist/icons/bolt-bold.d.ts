import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarBoltBold [size]="24" color="#ef4444" />`
 */
export declare class SolarBoltBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarBoltBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarBoltBold, "svg[solarBoltBold]", never, {}, {}, never, never, true, never>;
}
