import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarNotificationRemoveBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarNotificationRemoveBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarNotificationRemoveBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarNotificationRemoveBroken, "svg[solarNotificationRemoveBroken]", never, {}, {}, never, never, true, never>;
}
