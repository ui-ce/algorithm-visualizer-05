import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarPlayLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarPlayLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarPlayLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarPlayLinear, "svg[solarPlayLinear]", never, {}, {}, never, never, true, never>;
}
