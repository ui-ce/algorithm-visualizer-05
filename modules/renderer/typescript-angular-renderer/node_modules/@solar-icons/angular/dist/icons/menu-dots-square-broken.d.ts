import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarMenuDotsSquareBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarMenuDotsSquareBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarMenuDotsSquareBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarMenuDotsSquareBroken, "svg[solarMenuDotsSquareBroken]", never, {}, {}, never, never, true, never>;
}
