import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarPaperclipRounded2LineDuotone [size]="24" color="#ef4444" />`
 */
export declare class SolarPaperclipRounded2LineDuotone extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarPaperclipRounded2LineDuotone, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarPaperclipRounded2LineDuotone, "svg[solarPaperclipRounded2LineDuotone]", never, {}, {}, never, never, true, never>;
}
