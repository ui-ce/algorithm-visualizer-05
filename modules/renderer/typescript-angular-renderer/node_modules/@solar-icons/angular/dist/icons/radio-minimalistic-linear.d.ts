import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarRadioMinimalisticLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarRadioMinimalisticLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarRadioMinimalisticLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarRadioMinimalisticLinear, "svg[solarRadioMinimalisticLinear]", never, {}, {}, never, never, true, never>;
}
