import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarFileLeftBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarFileLeftBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarFileLeftBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarFileLeftBroken, "svg[solarFileLeftBroken]", never, {}, {}, never, never, true, never>;
}
