import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarHomeWiFiLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarHomeWiFiLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarHomeWiFiLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarHomeWiFiLinear, "svg[solarHomeWiFiLinear]", never, {}, {}, never, never, true, never>;
}
