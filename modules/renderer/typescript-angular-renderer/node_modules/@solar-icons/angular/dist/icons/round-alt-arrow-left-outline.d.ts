import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarRoundAltArrowLeftOutline [size]="24" color="#ef4444" />`
 */
export declare class SolarRoundAltArrowLeftOutline extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarRoundAltArrowLeftOutline, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarRoundAltArrowLeftOutline, "svg[solarRoundAltArrowLeftOutline]", never, {}, {}, never, never, true, never>;
}
