import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarClockSquareBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarClockSquareBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarClockSquareBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarClockSquareBroken, "svg[solarClockSquareBroken]", never, {}, {}, never, never, true, never>;
}
