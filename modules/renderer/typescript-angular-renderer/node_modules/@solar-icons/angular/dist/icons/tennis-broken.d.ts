import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarTennisBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarTennisBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarTennisBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarTennisBroken, "svg[solarTennisBroken]", never, {}, {}, never, never, true, never>;
}
