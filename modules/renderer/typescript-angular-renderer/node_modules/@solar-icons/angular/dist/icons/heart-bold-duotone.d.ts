import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarHeartBoldDuotone [size]="24" color="#ef4444" />`
 */
export declare class SolarHeartBoldDuotone extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarHeartBoldDuotone, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarHeartBoldDuotone, "svg[solarHeartBoldDuotone]", never, {}, {}, never, never, true, never>;
}
