import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarListUpMinimalisticOutline [size]="24" color="#ef4444" />`
 */
export declare class SolarListUpMinimalisticOutline extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarListUpMinimalisticOutline, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarListUpMinimalisticOutline, "svg[solarListUpMinimalisticOutline]", never, {}, {}, never, never, true, never>;
}
