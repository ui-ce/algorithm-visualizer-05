import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarHomeAddLineDuotone [size]="24" color="#ef4444" />`
 */
export declare class SolarHomeAddLineDuotone extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarHomeAddLineDuotone, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarHomeAddLineDuotone, "svg[solarHomeAddLineDuotone]", never, {}, {}, never, never, true, never>;
}
