import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarFilterBold [size]="24" color="#ef4444" />`
 */
export declare class SolarFilterBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarFilterBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarFilterBold, "svg[solarFilterBold]", never, {}, {}, never, never, true, never>;
}
