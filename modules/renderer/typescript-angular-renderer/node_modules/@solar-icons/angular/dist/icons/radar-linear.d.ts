import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarRadarLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarRadarLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarRadarLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarRadarLinear, "svg[solarRadarLinear]", never, {}, {}, never, never, true, never>;
}
