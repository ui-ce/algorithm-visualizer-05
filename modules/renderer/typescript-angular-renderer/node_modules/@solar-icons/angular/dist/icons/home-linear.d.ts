import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarHomeLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarHomeLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarHomeLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarHomeLinear, "svg[solarHomeLinear]", never, {}, {}, never, never, true, never>;
}
