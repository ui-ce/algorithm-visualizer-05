import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarRefreshLineDuotone [size]="24" color="#ef4444" />`
 */
export declare class SolarRefreshLineDuotone extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarRefreshLineDuotone, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarRefreshLineDuotone, "svg[solarRefreshLineDuotone]", never, {}, {}, never, never, true, never>;
}
