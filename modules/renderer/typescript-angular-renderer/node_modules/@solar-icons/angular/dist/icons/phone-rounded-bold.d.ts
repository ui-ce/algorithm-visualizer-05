import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarPhoneRoundedBold [size]="24" color="#ef4444" />`
 */
export declare class SolarPhoneRoundedBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarPhoneRoundedBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarPhoneRoundedBold, "svg[solarPhoneRoundedBold]", never, {}, {}, never, never, true, never>;
}
