import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarUndoLeftRoundBold [size]="24" color="#ef4444" />`
 */
export declare class SolarUndoLeftRoundBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarUndoLeftRoundBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarUndoLeftRoundBold, "svg[solarUndoLeftRoundBold]", never, {}, {}, never, never, true, never>;
}
