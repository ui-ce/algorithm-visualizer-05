import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarMoonLineDuotone [size]="24" color="#ef4444" />`
 */
export declare class SolarMoonLineDuotone extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarMoonLineDuotone, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarMoonLineDuotone, "svg[solarMoonLineDuotone]", never, {}, {}, never, never, true, never>;
}
