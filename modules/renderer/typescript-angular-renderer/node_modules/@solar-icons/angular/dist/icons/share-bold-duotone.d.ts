import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarShareBoldDuotone [size]="24" color="#ef4444" />`
 */
export declare class SolarShareBoldDuotone extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarShareBoldDuotone, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarShareBoldDuotone, "svg[solarShareBoldDuotone]", never, {}, {}, never, never, true, never>;
}
