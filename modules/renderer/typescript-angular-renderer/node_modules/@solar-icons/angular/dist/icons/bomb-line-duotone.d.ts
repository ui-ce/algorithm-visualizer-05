import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarBombLineDuotone [size]="24" color="#ef4444" />`
 */
export declare class SolarBombLineDuotone extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarBombLineDuotone, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarBombLineDuotone, "svg[solarBombLineDuotone]", never, {}, {}, never, never, true, never>;
}
