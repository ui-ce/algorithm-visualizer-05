import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarRefreshBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarRefreshBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarRefreshBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarRefreshBroken, "svg[solarRefreshBroken]", never, {}, {}, never, never, true, never>;
}
