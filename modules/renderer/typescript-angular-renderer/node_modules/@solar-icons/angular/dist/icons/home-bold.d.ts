import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarHomeBold [size]="24" color="#ef4444" />`
 */
export declare class SolarHomeBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarHomeBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarHomeBold, "svg[solarHomeBold]", never, {}, {}, never, never, true, never>;
}
