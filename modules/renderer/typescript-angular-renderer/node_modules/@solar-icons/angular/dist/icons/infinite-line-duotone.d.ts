import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarInfiniteLineDuotone [size]="24" color="#ef4444" />`
 */
export declare class SolarInfiniteLineDuotone extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarInfiniteLineDuotone, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarInfiniteLineDuotone, "svg[solarInfiniteLineDuotone]", never, {}, {}, never, never, true, never>;
}
