import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarHashtagOutline [size]="24" color="#ef4444" />`
 */
export declare class SolarHashtagOutline extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarHashtagOutline, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarHashtagOutline, "svg[solarHashtagOutline]", never, {}, {}, never, never, true, never>;
}
