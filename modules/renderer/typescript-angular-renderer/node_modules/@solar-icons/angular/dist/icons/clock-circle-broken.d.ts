import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarClockCircleBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarClockCircleBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarClockCircleBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarClockCircleBroken, "svg[solarClockCircleBroken]", never, {}, {}, never, never, true, never>;
}
