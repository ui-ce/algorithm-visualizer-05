import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarShieldMinimalisticBold [size]="24" color="#ef4444" />`
 */
export declare class SolarShieldMinimalisticBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarShieldMinimalisticBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarShieldMinimalisticBold, "svg[solarShieldMinimalisticBold]", never, {}, {}, never, never, true, never>;
}
