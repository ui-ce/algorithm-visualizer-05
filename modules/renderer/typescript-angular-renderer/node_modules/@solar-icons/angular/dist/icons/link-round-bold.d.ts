import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarLinkRoundBold [size]="24" color="#ef4444" />`
 */
export declare class SolarLinkRoundBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarLinkRoundBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarLinkRoundBold, "svg[solarLinkRoundBold]", never, {}, {}, never, never, true, never>;
}
