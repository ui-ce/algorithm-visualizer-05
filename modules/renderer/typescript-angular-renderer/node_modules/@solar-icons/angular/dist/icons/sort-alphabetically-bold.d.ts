import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarSortAlphabeticallyBold [size]="24" color="#ef4444" />`
 */
export declare class SolarSortAlphabeticallyBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarSortAlphabeticallyBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarSortAlphabeticallyBold, "svg[solarSortAlphabeticallyBold]", never, {}, {}, never, never, true, never>;
}
