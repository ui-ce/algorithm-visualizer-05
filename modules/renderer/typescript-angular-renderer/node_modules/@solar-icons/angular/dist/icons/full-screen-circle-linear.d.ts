import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarFullScreenCircleLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarFullScreenCircleLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarFullScreenCircleLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarFullScreenCircleLinear, "svg[solarFullScreenCircleLinear]", never, {}, {}, never, never, true, never>;
}
