import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarHomeAddLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarHomeAddLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarHomeAddLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarHomeAddLinear, "svg[solarHomeAddLinear]", never, {}, {}, never, never, true, never>;
}
