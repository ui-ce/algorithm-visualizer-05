import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarMinimizeOutline [size]="24" color="#ef4444" />`
 */
export declare class SolarMinimizeOutline extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarMinimizeOutline, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarMinimizeOutline, "svg[solarMinimizeOutline]", never, {}, {}, never, never, true, never>;
}
