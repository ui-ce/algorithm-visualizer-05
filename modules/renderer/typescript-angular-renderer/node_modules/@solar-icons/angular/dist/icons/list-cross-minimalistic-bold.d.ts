import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarListCrossMinimalisticBold [size]="24" color="#ef4444" />`
 */
export declare class SolarListCrossMinimalisticBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarListCrossMinimalisticBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarListCrossMinimalisticBold, "svg[solarListCrossMinimalisticBold]", never, {}, {}, never, never, true, never>;
}
