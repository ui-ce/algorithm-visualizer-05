import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarHomeWiFiAngleBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarHomeWiFiAngleBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarHomeWiFiAngleBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarHomeWiFiAngleBroken, "svg[solarHomeWiFiAngleBroken]", never, {}, {}, never, never, true, never>;
}
