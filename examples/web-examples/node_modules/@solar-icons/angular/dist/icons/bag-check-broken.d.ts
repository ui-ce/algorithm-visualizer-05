import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarBagCheckBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarBagCheckBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarBagCheckBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarBagCheckBroken, "svg[solarBagCheckBroken]", never, {}, {}, never, never, true, never>;
}
