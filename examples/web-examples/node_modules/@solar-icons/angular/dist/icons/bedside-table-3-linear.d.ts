import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarBedsideTable3Linear [size]="24" color="#ef4444" />`
 */
export declare class SolarBedsideTable3Linear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarBedsideTable3Linear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarBedsideTable3Linear, "svg[solarBedsideTable3Linear]", never, {}, {}, never, never, true, never>;
}
