import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarSquareBottomDownBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarSquareBottomDownBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarSquareBottomDownBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarSquareBottomDownBroken, "svg[solarSquareBottomDownBroken]", never, {}, {}, never, never, true, never>;
}
