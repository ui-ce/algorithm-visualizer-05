import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarMagnetBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarMagnetBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarMagnetBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarMagnetBroken, "svg[solarMagnetBroken]", never, {}, {}, never, never, true, never>;
}
