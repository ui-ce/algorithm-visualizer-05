import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarBodyShapeMinimalisticLineDuotone [size]="24" color="#ef4444" />`
 */
export declare class SolarBodyShapeMinimalisticLineDuotone extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarBodyShapeMinimalisticLineDuotone, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarBodyShapeMinimalisticLineDuotone, "svg[solarBodyShapeMinimalisticLineDuotone]", never, {}, {}, never, never, true, never>;
}
