import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarEyeClosedLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarEyeClosedLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarEyeClosedLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarEyeClosedLinear, "svg[solarEyeClosedLinear]", never, {}, {}, never, never, true, never>;
}
