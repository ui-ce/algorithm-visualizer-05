import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarHeartAngleBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarHeartAngleBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarHeartAngleBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarHeartAngleBroken, "svg[solarHeartAngleBroken]", never, {}, {}, never, never, true, never>;
}
