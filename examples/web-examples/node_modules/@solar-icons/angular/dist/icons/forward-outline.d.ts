import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarForwardOutline [size]="24" color="#ef4444" />`
 */
export declare class SolarForwardOutline extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarForwardOutline, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarForwardOutline, "svg[solarForwardOutline]", never, {}, {}, never, never, true, never>;
}
