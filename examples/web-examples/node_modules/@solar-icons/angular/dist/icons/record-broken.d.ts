import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarRecordBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarRecordBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarRecordBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarRecordBroken, "svg[solarRecordBroken]", never, {}, {}, never, never, true, never>;
}
