import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarPlaylistMinimalisticLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarPlaylistMinimalisticLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarPlaylistMinimalisticLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarPlaylistMinimalisticLinear, "svg[solarPlaylistMinimalisticLinear]", never, {}, {}, never, never, true, never>;
}
