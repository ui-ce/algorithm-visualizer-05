import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarTextCircleBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarTextCircleBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarTextCircleBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarTextCircleBroken, "svg[solarTextCircleBroken]", never, {}, {}, never, never, true, never>;
}
