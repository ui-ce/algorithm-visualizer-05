import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarListBold [size]="24" color="#ef4444" />`
 */
export declare class SolarListBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarListBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarListBold, "svg[solarListBold]", never, {}, {}, never, never, true, never>;
}
