import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarGalleryAddBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarGalleryAddBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarGalleryAddBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarGalleryAddBroken, "svg[solarGalleryAddBroken]", never, {}, {}, never, never, true, never>;
}
