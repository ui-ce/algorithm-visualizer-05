import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarFireMinimalisticBold [size]="24" color="#ef4444" />`
 */
export declare class SolarFireMinimalisticBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarFireMinimalisticBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarFireMinimalisticBold, "svg[solarFireMinimalisticBold]", never, {}, {}, never, never, true, never>;
}
