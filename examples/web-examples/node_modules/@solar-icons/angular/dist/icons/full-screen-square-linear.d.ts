import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarFullScreenSquareLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarFullScreenSquareLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarFullScreenSquareLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarFullScreenSquareLinear, "svg[solarFullScreenSquareLinear]", never, {}, {}, never, never, true, never>;
}
