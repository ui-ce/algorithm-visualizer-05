import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarAirbudsCaseLineDuotone [size]="24" color="#ef4444" />`
 */
export declare class SolarAirbudsCaseLineDuotone extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarAirbudsCaseLineDuotone, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarAirbudsCaseLineDuotone, "svg[solarAirbudsCaseLineDuotone]", never, {}, {}, never, never, true, never>;
}
