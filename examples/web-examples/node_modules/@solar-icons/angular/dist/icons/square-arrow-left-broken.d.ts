import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarSquareArrowLeftBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarSquareArrowLeftBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarSquareArrowLeftBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarSquareArrowLeftBroken, "svg[solarSquareArrowLeftBroken]", never, {}, {}, never, never, true, never>;
}
