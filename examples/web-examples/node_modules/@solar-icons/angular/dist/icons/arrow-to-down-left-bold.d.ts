import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarArrowToDownLeftBold [size]="24" color="#ef4444" />`
 */
export declare class SolarArrowToDownLeftBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarArrowToDownLeftBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarArrowToDownLeftBold, "svg[solarArrowToDownLeftBold]", never, {}, {}, never, never, true, never>;
}
