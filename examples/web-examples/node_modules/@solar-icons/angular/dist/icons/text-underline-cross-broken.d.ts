import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarTextUnderlineCrossBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarTextUnderlineCrossBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarTextUnderlineCrossBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarTextUnderlineCrossBroken, "svg[solarTextUnderlineCrossBroken]", never, {}, {}, never, never, true, never>;
}
