import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarChecklistLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarChecklistLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarChecklistLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarChecklistLinear, "svg[solarChecklistLinear]", never, {}, {}, never, never, true, never>;
}
