import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarChecklistOutline [size]="24" color="#ef4444" />`
 */
export declare class SolarChecklistOutline extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarChecklistOutline, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarChecklistOutline, "svg[solarChecklistOutline]", never, {}, {}, never, never, true, never>;
}
