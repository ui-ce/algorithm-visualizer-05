import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarBombLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarBombLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarBombLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarBombLinear, "svg[solarBombLinear]", never, {}, {}, never, never, true, never>;
}
