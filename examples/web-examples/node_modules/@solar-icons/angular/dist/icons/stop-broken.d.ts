import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarStopBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarStopBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarStopBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarStopBroken, "svg[solarStopBroken]", never, {}, {}, never, never, true, never>;
}
