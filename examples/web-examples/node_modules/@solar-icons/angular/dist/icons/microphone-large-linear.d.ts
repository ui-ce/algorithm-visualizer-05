import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarMicrophoneLargeLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarMicrophoneLargeLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarMicrophoneLargeLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarMicrophoneLargeLinear, "svg[solarMicrophoneLargeLinear]", never, {}, {}, never, never, true, never>;
}
