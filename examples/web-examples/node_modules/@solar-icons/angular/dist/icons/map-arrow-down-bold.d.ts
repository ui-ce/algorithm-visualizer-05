import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarMapArrowDownBold [size]="24" color="#ef4444" />`
 */
export declare class SolarMapArrowDownBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarMapArrowDownBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarMapArrowDownBold, "svg[solarMapArrowDownBold]", never, {}, {}, never, never, true, never>;
}
