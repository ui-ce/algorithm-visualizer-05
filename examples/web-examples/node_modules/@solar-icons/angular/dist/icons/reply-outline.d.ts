import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarReplyOutline [size]="24" color="#ef4444" />`
 */
export declare class SolarReplyOutline extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarReplyOutline, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarReplyOutline, "svg[solarReplyOutline]", never, {}, {}, never, never, true, never>;
}
