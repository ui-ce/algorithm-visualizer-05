import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarChatRoundBold [size]="24" color="#ef4444" />`
 */
export declare class SolarChatRoundBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarChatRoundBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarChatRoundBold, "svg[solarChatRoundBold]", never, {}, {}, never, never, true, never>;
}
