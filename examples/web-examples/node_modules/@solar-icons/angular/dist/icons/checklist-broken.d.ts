import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarChecklistBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarChecklistBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarChecklistBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarChecklistBroken, "svg[solarChecklistBroken]", never, {}, {}, never, never, true, never>;
}
