import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarMapArrowLeftBold [size]="24" color="#ef4444" />`
 */
export declare class SolarMapArrowLeftBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarMapArrowLeftBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarMapArrowLeftBold, "svg[solarMapArrowLeftBold]", never, {}, {}, never, never, true, never>;
}
