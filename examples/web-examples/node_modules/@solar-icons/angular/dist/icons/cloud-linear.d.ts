import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarCloudLinear [size]="24" color="#ef4444" />`
 */
export declare class SolarCloudLinear extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarCloudLinear, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarCloudLinear, "svg[solarCloudLinear]", never, {}, {}, never, never, true, never>;
}
