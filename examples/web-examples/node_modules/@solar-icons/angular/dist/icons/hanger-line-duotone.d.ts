import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarHangerLineDuotone [size]="24" color="#ef4444" />`
 */
export declare class SolarHangerLineDuotone extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarHangerLineDuotone, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarHangerLineDuotone, "svg[solarHangerLineDuotone]", never, {}, {}, never, never, true, never>;
}
