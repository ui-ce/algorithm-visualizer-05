import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarRoutingBroken [size]="24" color="#ef4444" />`
 */
export declare class SolarRoutingBroken extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarRoutingBroken, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarRoutingBroken, "svg[solarRoutingBroken]", never, {}, {}, never, never, true, never>;
}
