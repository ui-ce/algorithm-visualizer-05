import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarListOutline [size]="24" color="#ef4444" />`
 */
export declare class SolarListOutline extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarListOutline, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarListOutline, "svg[solarListOutline]", never, {}, {}, never, never, true, never>;
}
