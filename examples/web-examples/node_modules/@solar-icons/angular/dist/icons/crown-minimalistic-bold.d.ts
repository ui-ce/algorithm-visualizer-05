import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarCrownMinimalisticBold [size]="24" color="#ef4444" />`
 */
export declare class SolarCrownMinimalisticBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarCrownMinimalisticBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarCrownMinimalisticBold, "svg[solarCrownMinimalisticBold]", never, {}, {}, never, never, true, never>;
}
