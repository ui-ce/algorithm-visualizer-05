import { IconBase } from '../lib/icon-base';
import * as i0 from "@angular/core";
/**
 * ![img](data:image/svg+xml;base64,)
 *
 * @usage
 * `<svg solarMapArrowUpBold [size]="24" color="#ef4444" />`
 */
export declare class SolarMapArrowUpBold extends IconBase {
    static ɵfac: i0.ɵɵFactoryDeclaration<SolarMapArrowUpBold, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<SolarMapArrowUpBold, "svg[solarMapArrowUpBold]", never, {}, {}, never, never, true, never>;
}
